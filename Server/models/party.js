...
module.exports = class Party{
    ...
    async createParty(){
        try{
            const result = await pool.query('INSERT INTO party (name_party) VALUES ($1)',[this.name]);
    ...
    async modifyNameParty(id){
        try{
            const result = await pool.query(`UPDATE party SET
                name_party = ($1) 
                WHERE id = $2 RETRUNING *`,[this.name,id]);
            return result.rows[0];
        }catch(err){
    ...
    static async deleteParty(id){
        try{
            const result = await pool.query(`DELETE FROM party
                WHERE id = $1 RETRUNING *`,[id]);
            return result.rows[0];
        }catch(err){
